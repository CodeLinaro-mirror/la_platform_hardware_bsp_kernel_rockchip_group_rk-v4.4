...
static int cw_get_voltage(struct cw_battery *cw_bat)
{
	...
	if (cw_bat->plat_data.divider_res1 &&
		...
		/*
		 * If dual battery, through the hardware divide voltage,
		 * the voltage that drive collection is half of the battery.
		 */
		voltage = voltage * 2;
	}
...
static int cw2015_parse_dt(struct cw_battery *cw_bat)
{
	...
	if (!ret && value) {
		hw_id_io = gpiod_get_optional(dev, "hw-id0", GPIOD_IN);
		if (!hw_id_io)
			return -EINVAL;
		if (IS_ERR(hw_id_io))
			return PTR_ERR(hw_id_io);

		hw_id0_val = gpiod_get_value(hw_id_io);
		gpiod_put(hw_id_io);
		if (hw_id0_val < 0)
			return -EINVAL;

		hw_id_io = gpiod_get_optional(dev, "hw-id1", GPIOD_IN);
		if (!hw_id_io)
			return -EINVAL;
		if (IS_ERR(hw_id_io))
			return PTR_ERR(hw_id_io);

		hw_id1_val = gpiod_get_value(hw_id_io);
		gpiod_put(hw_id_io);
		if (hw_id1_val < 0)
			return -EINVAL;
